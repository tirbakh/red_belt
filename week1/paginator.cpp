...
class Page {
public:
    Page(Iterator _first, Iterator _last) : first(_first), last(_last) {}
...
class Paginator {
public:
    Paginator(Iterator begin, Iterator end, long _pageSize) : pageSize(_pageSize) {
        for(auto it = begin; it != end; it += min(distance(it, end), pageSize)) {
            auto inc = min(distance(it, end), pageSize);
            auto last_page = it + inc;
            pages.push_back({it, last_page});
        }
    }
